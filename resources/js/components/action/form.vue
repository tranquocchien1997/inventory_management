<template>
    <div v-if="(hasPermission('detail') && type != 'create') || ( hasPermission('create') && type == 'create')" class="card card-primary card-outline">
        ...
        <div v-if="showForm" class="card-body">
            <div class="row">
                <div v-for="item in configModel" :class="item.type == 'textArea' ? 'col-md-6 form-group' : 'col-md-3 form-group'" v-if="checkShowInput(item)">
        ...
        methods: {
            ...
            checkShowInput(item){
                if (item.model == 'inventory_product_id'){
                    if (!this.model['inventory_id']) return false

                    let config = this.configModel.find(item => item.model == 'inventory_product_id')

                    const inventory = reference.inventory.find(item => item.id == this.model['inventory_id'])

                    if (inventory && inventory.type == ENROL_INVENTORY_TYPE_ID){
                ...
                if (item.model == 'inventory_receive_id'){
                    if (!this.model['action_type_id']) return false

                    let config = this.configModel.find(item => item.model == 'inventory_receive_id')

                    const actionType = reference.actionType.find(item => item.id == this.model['action_type_id'])
