        ...
        <div v-if="showForm" class="card-body">
            <div class="row">
                <div v-for="item in configModel" :class="item.type == 'textArea' ? 'col-md-6 form-group' : 'col-md-3 form-group'">
                    <bBtn
                        v-if="item.type == 'btn'"
                        title="Sao chép từ dự kiến"
                        color="primary"
                        className="float-left "
                        icon="fa fa-copy"
                        :onClick="copyEstimate" />
                    <InputField v-else-if="item" :item="item" :model="model" :data="config.getDropdownField(item)"/>
                </div>
            </div>
        </div>
        <!-- /.card-body -->
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            copyEstimate(){
                this.model.date_delivered = this.model.date_delivery
                this.model.quantity_delivered = this.model.quantity_delivery
                this.model.date_amount_receive = this.model.estimate_date_amount_receive
                this.model.amount_receive = this.model.estimate_amount_receive
            }
        },
        watch: {
